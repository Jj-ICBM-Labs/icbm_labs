...
export default function Nav() {
    ...
    useEffect(() => {
        if (item.current) {
            if (open) {
                item.current.style.transform = 'translate(0, 311px)';
            } else {
                item.current.style.transform = 'translate(0, -20px)';
            }
        }
    }, [open]);
    ...
    return (
        <div className={styles.nav}>
            ...
            <div ref={item} className={styles.items}>
                ...
                <Link onClick={() => setOpen(false)} href={"#intro"}>
                    소개
                </Link>
                <Link onClick={() => setOpen(false)} href={"#research"}>
                    연구
                </Link>
                <Link onClick={() => setOpen(false)} href={"#welfare"}>
                    복지
                </Link>
                <Link onClick={() => setOpen(false)} href={"#icbm"}>
                    지원
                </Link>
            </div>
        </div>
